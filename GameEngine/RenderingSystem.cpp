...
{
	// Creates a pointer (by using "new", remember to "delete" after!)
...
void RenderingSystem::tick(ECS::World* world, float deltaTime)
{
	...
	// Pass in the components by their stucts/classes (<>)
	world->each<Transfrom, Sprite>(
		[&](ECS::Entity* entity,
			ECS::ComponentHandle<Transfrom> transform,
